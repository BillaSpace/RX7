...
from pytgcalls.types.input_stream.quality import MediumQualityVideo, StudioQualityAudio
...
class Call(PyTgCalls):
    ...
    async def speedup_stream(self, chat_id: int, file_path, speed, playing):
        ...
        stream = (
            AudioVideoPiped(
                out,
                audio_parameters=StudioQualityAudio(),
    ...
    async def skip_stream(
        ...
        if video:
            ...
            stream = AudioPiped(link, audio_parameters=StudioQualityAudio())
    ...
    async def join_call(
        ...
        if video:
            ...
            stream = (
                ...
                if video
                else AudioPiped(link, audio_parameters=StudioQualityAudio())
            )
        try:
            await assistant.join_group_call(
                chat_id,
        ...
        try:
            ...
            if "live_" in queued:
                ...
                stream = (
                    ...
                    else AudioPiped(videoid, audio_parameters=StudioQualityAudio())
                )
                try:
                    await client.change_stream(chat_id, stream)
                except:
